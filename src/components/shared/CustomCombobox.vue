...
import { TagsInput, TagsInputInput } from '@/components/ui/tags-input/index.js'
...
const filteredOptions = computed(() => {
  const filteredBySelected = props.options.filter(i => !modelValue.value.includes(i.value));
  if (searchTerm.value == '') return filteredBySelected;
  return filteredBySelected.filter(i => i.label.toLowerCase().includes(searchTerm.value.toLowerCase()));
});
        ...
        <ComboboxAnchor as-child>
          <div class="flex items-center justify-end w-full">
            <ComboboxInput :placeholder="props.placeholder" as-child>
              <TagsInputInput class="w-full px-3 border-0 focus:ring-0 py-0" :class="modelValue.length > 0 ? '' : ''"
                @keydown.enter.prevent @focus="toggleOpen" />
            </ComboboxInput>
          </div>
        </ComboboxAnchor>
        <ComboboxPortal>
